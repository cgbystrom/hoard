...
module.exports =
    ...
    'test create()': (beforeExit) ->
        called = false
        filename = 'spec/testcreate.hoard'
        ...
        hoard.create filename, [[1, 60], [10, 600]], 0.5, (err) ->
            ...
            whisperFile = fs.readFileSync('spec/testcreate.whisper')
    ...
    'test update()': (beforeExit) ->
        called = false
        filename = 'spec/testupdate.hoard'
        if path.existsSync(filename) then fs.unlinkSync(filename)
    ...
    'test updateMany()': (beforeExit) ->
        called = false
        filename = 'spec/testupdatemany.hoard'
        if path.existsSync(filename) then fs.unlinkSync(filename)

        tsData = JSON.parse(fs.readFileSync('spec/timeseriesdata.json', 'utf8'))
